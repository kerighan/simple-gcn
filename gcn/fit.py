...
def fit_3_layer(
    ...
    W_1 = tf.Variable(tf.random.normal((n_features, latent_dim)))
    W_2 = tf.Variable(tf.random.normal((latent_dim, latent_dim)))
    W_3 = tf.Variable(tf.random.normal((latent_dim, latent_dim)))
    O = tf.Variable(tf.random.normal((latent_dim, n_labels)))
    agg_1 = tf.sparse.sparse_dense_matmul(A, X)
    var_list = [W_1, W_2, W_3, O]
    ...
    for i in t:
        with tf.GradientTape() as tape:
            ...
            agg_3 = tf.sparse.sparse_dense_matmul(A, layer_2)
            layer_3 = activation(tf.matmul(agg_3, W_3))

            out = tf.nn.softmax(tf.matmul(layer_3, O), axis=-1)
